...
1. Use GitHub Issues to document requirements (see [Chapter 20: Software development practices from *R Packages (2e)* by Hadley Wickham and Jennifer Bryan](https://r-pkgs.org/software-development-practices.html)).
2. Use {testthat} to verify the implementation of those requirements (see the [Testing section of *R Packages (2e)* by Hadley Wickham and Jennifer Bryan](https://r-pkgs.org/testing-basics.html)).
...
A sample [GitHub Action to generate a report is included in this repository](https://github.com/Gilead-BioStats/qcthat/blob/dev/.github/workflows/qcthis.yaml) (coming soon to the package itself). This is the process that it follows:
...
For example, this is the output for a recent report for this {qcthat} repository.
...
3. **🧪 Write Tests:** Quality Control Programmer writes needed tests and includes a link to the issue by including `(#{issue_number})` in the test description.
4. **🤖 Release Package:** Upon a GitHub release, all tests are automatically ran and a summary report linking issues with associated tests is created and attached to the release. 

...
Load the package environment with `pkgload::load_all()` and pass it via `envPkg` to `MapTestFilesToPotentialIssues()` to augment test-file blame with source-line coverage from `covr`. This discovers additional potential issues from commits that touched source code exercised by each test, not just the test files themselves.
...
dfPotentialIssues <- MapTestFilesToPotentialIssues(
  dfFileTests, envPkg = env, dfIssueCommitsLong = dfIssueCommitsLong
)

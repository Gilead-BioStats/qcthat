# Read a JSON file as an IssueTestMatrix

Read a JSON file that was saved with
[`SaveAsJSON.qcthat_IssueTestMatrix()`](https://gilead-biostats.github.io/qcthat/reference/SaveAsJSON.md)
and return it as a `qcthat_IssueTestMatrix` object.

## Usage

``` r
ReadJSONAsIssueTestMatrix(strJSONPath)
```

## Arguments

- strJSONPath:

  (`length-1 character`) File path to save or read JSON.

## Value

A `qcthat_IssueTestMatrix` object.

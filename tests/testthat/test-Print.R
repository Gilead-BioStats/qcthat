test_that("Printing a generic qcthat_object returns input invisibly", {
  obj <- structure(
...
test_that("MakeKeyItem works", {
  expect_snapshot({
    MakeKeyItem("open")
  })
})

test_that("ChooseEmoji switches to ASCII if emoji not allowed", {
  expect_identical(
...
test_that("GetChrCode returns the expected code", {
  expect_identical(
    GetChrCode("box"),
    "\U2588"
  )
...
test_that("FinalizeTree adds tree characters correctly", {
